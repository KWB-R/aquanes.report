...
calculate_operational_parameters <- function(df,
                                 calc_list = list(Redox_Out = "(Redox_Out1+Redox_Out2)/2",
...
operation_matrix <- operation %>%
                    tidyr::spread_(key_col = "ParameterCode",
...
operation_calc_tidy <- tidyr::gather_(operation_calc,
...
plot_calculated_operational_timeseries <- function(df) {
  ...
  for (i in seq_along(calculated_paras)) {
    ...
    if (n_measurements > 0) {
      ...
      g1 <- ggplot2::ggplot(df, ggplot2::aes_string(x = "DateTime",
                                                       y = "ParameterValue")) +
        ggforce::facet_wrap_paginate(~ParameterLabel,
                                     nrow = 1,
                                     ncol = 1,
                                     scales = "free_y",
                                     page = i) +
