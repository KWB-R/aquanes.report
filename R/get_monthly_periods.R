...
data.frame(year_month = format(month_start,format = "%Y-%m"),
           start = month_start,
           end = month_end,
...
get_rawfilespaths_for_month <- function(monthly_period = get_monthly_periods()[1,],
            raw_data_dir = system.file("shiny/berlin_t/data/operation",
...
if(any(min_offset_bool)) {
  ...
  warning(sprintf("Importing period: '%s'
...
dates_to_grap <- c(min_offset,
                   as.character(seq(monthly_period$start,
                                monthly_period$end,
                                by = "days")),
                   max_offset)
